    ...
    public partial class close_traveling_rf : Form
    {
        ...
        private int raschetGas(bool isClose = false)
        {
            try
            {
                ...
                traveling.Z_gas_1 = 0.00m;
                for(int i = 0; i < traveling.listGasstations.Count; i++)
                    if (traveling.listGasstations[i].date_gas.Value.Date == traveling.date_traveling.Date)
                        traveling.Z_gas_1 += traveling.listGasstations[i].Enter_gas;
                traveling.E_gas_1 = traveling.S_gas_1 - traveling.T_gas_1 + traveling.Z_gas_1;
                traveling.S_gas_2 = traveling.E_gas_1;
                textBox17.Text = Convert.ToString(traveling.S_gas_2);
                textBox9.Text = Convert.ToString(traveling.E_gas_1);
                traveling.Z_gas_2 = 0.00m;
                for (int i = 0; i < traveling.listGasstations.Count; i++)
                    if (traveling.listGasstations[i].date_gas.Value.Date == traveling.date_traveling.Date.AddDays(1))
